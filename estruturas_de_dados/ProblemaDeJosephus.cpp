...
void PegarDados(Soldado* pSoldado){
	printf("Informe o nome do Soldado: ");
	scanf("%s",&pSoldado->Nome);
	printf("Informe a posicao militar do soldado: ");
	scanf("%s",&pSoldado->PosMilitar);
	fflush(stdin);
	printf("Informe o valor de dinheiro que o soldado possui: ");
	scanf("%f", &pSoldado->Dinheiro);
	printf("Informe o numero do soldado: ");
...
void ImprimeLista(queue* s){
	...
	while(pAux != NULL){
		printf("Nome: %s\tPosicao Militar: %s\tDinheiro: %0.2f\tNumero: %d\n",
			pAux->soldado.Nome,
			pAux->soldado.PosMilitar,
			pAux->soldado.Dinheiro,
			pAux->soldado.numero);
		pAux = pAux->proximo;
	}
	printf(" ===================================\n");
